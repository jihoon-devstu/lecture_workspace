...
public class ProductDAO {
	...
	public List selectRecentList(int limit) {
		...
		StringBuffer sql = new StringBuffer();
		//상품과 서브 분류 조인 후 , 1:多 관계에 있는 product_img , product_size , product_color
		//서브 쿼리로 가져오자.
		sql.append("select product_id, product_name, brand, discount, introduce, detail");
		...
		try {
			...
			rs = pstmt.executeQuery();
			while(rs.next()) {
				Product product = new Product(); //현재 empty
